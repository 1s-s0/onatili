        ...
        {exercise.options.map((option, index) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                'p-6 rounded-2xl border-2 font-semibold text-lg transition-all flex items-center justify-between group',
                buttonStyle,
          ...
          <p className="text-xl text-white text-center leading-relaxed">
            {exercise.question.split('___').map((part, index, arr) => (
              <span key={index}>
                {part}
                {index < arr.length - 1 && (
                  <span className="inline-block min-w-[120px] border-b-2 border-lime-500/50 mx-1">
                    {selectedOption ? (
                      <motion.span
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className={cn(
                          'inline-block px-4 py-2 rounded-xl font-semibold',
                          isSubmitted
        ...
        {exercise.options.map((option, index) => {
          ...
          return (
            <motion.button
              ...
              disabled={isSubmitted}
              className={cn(
                'p-6 rounded-2xl border-2 font-semibold text-lg transition-all flex items-center justify-between',
                buttonStyle,
                !isSubmitted && 'hover:scale-105 hover:shadow-xl'
              )}
            >
